    ...
    <div class="min-h-screen flex flex-col">
        ...
        <main class="flex-1 flex items-center justify-center px-6 py-12 relative">
            ...
            <div class="w-full max-w-md space-y-8">
                ...
                <form action="{{ route('password.verifyOtp') }}" method="POST" class="space-y-6" x-data="otpInput()">
                    @csrf
                ...
                {{-- Reenviar código --}}
                <form action="{{ route('password.resendOtp') }}" method="POST" class="text-center">
                    @csrf
                    <button type="submit" class="text-[15px] font-normal underline hover:text-primary">
                        Reenviar código
                    </button>
                </form>
            </div>
